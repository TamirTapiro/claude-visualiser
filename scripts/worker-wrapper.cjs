...
var WORKER_PATH = path.join(__dirname, "worker-service.cjs");
var child = null;
var shuttingDown = false;
function log(msg) {
  console.log(`[${(/* @__PURE__ */ new Date()).toISOString()}] [vis-wrapper] ${msg}`);
}
...
async function killWorker() {
  ...
  if (IS_WINDOWS) {
    try {
      execSync(`taskkill /PID ${pid} /T /F`, { timeout: 1e4, stdio: "ignore" });
    } catch (e) {
